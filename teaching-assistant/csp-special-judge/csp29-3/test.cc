...
struct User {
  ...
  User() : id(), right{} {}
  bool operator<(const User &w) const { return id < w.id; }
};
struct Expr {
  int attr, val;
  bool type;
...
{
  if (e.type && usr.right.count(e.attr) && usr.right[e.attr] == e.val)
    return true;
  else if (!e.type && usr.right.count(e.attr) && usr.right[e.attr] != e.val)
    return true;
  else
    return false;
}
...
void build(int u, string &op, int &idx) // 建立表达式树
{
  ...
  } else // 不是逻辑符号，也不是括号，那就是数字，是叶子结点
  {
    exprs[u].isatom = true;
    int ind = idx;
    while (ind < op.size() && op[ind] != '~' && op[ind] != ':')
      ind++;
    exprs[u].attr = stoi(op.substr(idx, ind - idx));
    if (op[ind] == ':')
      exprs[u].type = 1; // :是1，~是0
    else
      exprs[u].type = 0;
    int end = ind;
    while (end < op.size() && op[end] != ')')
      end++;
...
bool judge(User &usr, int u) // 递归判定用户是否满足表达式
{
  auto &e = exprs[u];
  if (e.isatom)
    return judge_atom(usr, e); // 叶子节点直接判定
  else                         // 否则返回左右子树的结果的运算
  {
    if (e.type)
      return judge(usr, ls) | judge(usr, rs); // 或
    else
      return judge(usr, ls) & judge(usr, rs); // 且
  }
}
...
int main() {
  ...
  sort(users.begin(),
       users.end()); // 按照dn从小到大排序，后面遍历完可以直接输出结果了
  int m;
  cin >> m;
  string op;
  for (int i = 0; i < m; i++) {
...
// 作者：这道题有点难耶
// 链接：https://www.acwing.com/solution/content/212325/
// 来源：AcWing
